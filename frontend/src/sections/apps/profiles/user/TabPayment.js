          ...
          <Grid container spacing={2.5}>
                  ...
                  <TableBody>
                    {list?.invoice_detail?.map((row, index) => (
                      <TableRow key={row.name} sx={{ '&:last-child td, &:last-child th': { border: 0 } }}>
                        ...
                        <TableCell>{row.PaymentHistory}</TableCell>
                        <TableCell>{row.PaymentAmount}</TableCell>
                        <TableCell>{row.DeductionHistory}</TableCell>
                        <TableCell align="right">{row.DeductionAmount}</TableCell>
                        <TableCell align="right">{Number(row.price).toFixed(2)}원</TableCell>
                        <TableCell align="right">{Number(row.price * row.qty).toFixed(2)}원</TableCell>
                      </TableRow>
                    ))}
                  </TableBody>
                </Table>
            ...
            <Grid item xs={12}>
              <Stack direction="row" spacing={1}>
                ...
                <Typography>귀하의 노고에 감사드립니다.</Typography>
              </Stack>
            </Grid>
          </Grid>
        </Box>
